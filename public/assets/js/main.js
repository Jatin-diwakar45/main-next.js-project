(function ($) {
	...
	*/
	...
	= copyright year
	-------------------------------------------*/
	var yearSpan = document.getElementById("copyright_year");
	if (yearSpan) {
	...
	function updateActiveMenu() {
		...
		// Strip anchor hashes if present (e.g. contact.html#form)
		if (page.indexOf("#") !== -1) {
			page = page.split("#")[0];
		}
		// Strip query params
		...
		var $link = $('.main-menu a[href="' + page + '"]');

		if ($link.length) {
			$link.closest('li').addClass('active');
			$link.parents('.menu-item-has-children').addClass('active');
		} else {
			// Fallback for home if strict match fails (e.g. if path is just /)
			if (page === "index.html") {
				$('.main-menu a[href="index.html"]').closest('li').addClass('active');
			}
		}
	}
	updateActiveMenu();
